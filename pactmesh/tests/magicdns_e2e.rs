...
async fn setup_two_named_members() -> (tempfile::TempDir, String, MemberCert, MemberCert) {
    ...
    cert_a.details.hostname = Some(pactmesh::trust::HostnameLabel::try_from_str("laptop").unwrap());
    cert_b.details.hostname = Some(pactmesh::trust::HostnameLabel::try_from_str("server").unwrap());
