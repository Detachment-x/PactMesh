use pactmesh::proto::peer_rpc::{HandshakeRequest, PeerConnNoiseMsg3Pb, SecureAuthLevel};
use prost::Message;
...
fn test_noise_msg2_round_trip_with_member_cert_cbor() {
    let msg = pactmesh::proto::peer_rpc::PeerConnNoiseMsg2Pb {
        b_network_name: "office-net".to_owned(),
        role_hint: 1,
        action: pactmesh::proto::peer_rpc::PeerConnSessionActionPb::Join as i32,
    ...
    let encoded = msg.encode_to_vec();
    let decoded =
        pactmesh::proto::peer_rpc::PeerConnNoiseMsg2Pb::decode(encoded.as_slice()).unwrap();
