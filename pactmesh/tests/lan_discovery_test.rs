...
use pactmesh::trust::lan_discovery::{
...
async fn test_lan_discovery_happy_path_applies_newer_state_and_exposes_hint() {
    ...
    let report = apply_lan_response(
        ...
        &root.id(),
        &network_id(NETWORK),
        decode_lan_response(&pactmesh::trust::lan_discovery::encode_lan_response(
            &response,
        ))
        .unwrap(),
        None,
...
async fn test_lan_discovery_rejects_wrong_scope_and_tamper() {
    ...
    let wrong_domain_response = pactmesh::trust::lan_discovery::LanNetworkStateResponse {
        protocol_version: pactmesh::trust::lan_discovery::LAN_DISCOVERY_PROTOCOL_VERSION,
    ...
    assert!(matches!(
        err,
        LanDiscoveryError::Receive(pactmesh::trust::NetworkStateReceiveError::TrustDomainMismatch)
    ));

    let wrong_network_response = pactmesh::trust::lan_discovery::LanNetworkStateResponse {
        protocol_version: pactmesh::trust::lan_discovery::LAN_DISCOVERY_PROTOCOL_VERSION,
    ...
    assert!(matches!(
        ...
        LanDiscoveryError::Receive(
            pactmesh::trust::NetworkStateReceiveError::NetworkLocalIdMismatch
        )
    ));

    let mut tampered = state(&root, NETWORK, 2, Vec::new());
    tampered.details.payload.acl.push(0xAA);
    let tampered_response = pactmesh::trust::lan_discovery::LanNetworkStateResponse {
        protocol_version: pactmesh::trust::lan_discovery::LAN_DISCOVERY_PROTOCOL_VERSION,
    ...
    assert!(matches!(
        err,
        LanDiscoveryError::Receive(pactmesh::trust::NetworkStateReceiveError::PoolApply(
            pactmesh::trust::pool::PoolApplyError::BadSignature
        ))
    ));
}
