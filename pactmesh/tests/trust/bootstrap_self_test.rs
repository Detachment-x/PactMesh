...
fn test_bootstrap_self_is_idempotent_for_same_device_key() {
    ...
    assert_eq!(
        read_member_cert(dir.path(), &domain_id).fingerprint(),
        cert.fingerprint()
    );
    assert_eq!(
        read_state(dir.path(), &domain_id)
            .details
            .payload
            .member_cert_index
            .len(),
...
fn test_bootstrap_self_rejects_mismatched_existing_cert_domain() {
    ...
    let other_cert = pactmesh::trust::UnsignedMemberCert {
        trust_domain_id: other_root.id(),
        network_local_id: pactmesh::trust::NetworkLocalId::try_from_str("office-net").unwrap(),
...
fn test_bootstrap_self_rejects_corrupt_existing_member_cert() {
    ...
    std::fs::write(
        &cert_path,
        pactmesh::trust::wrap_armored("PNW-MEMBER-CERT", b"not-cbor"),
    )
    .unwrap();
...
fn test_bootstrap_self_device_id_points_to_pk_self() {
    ...
    let pk_pem = std::fs::read_to_string(
        config_home(dir.path()).join("privateNetwork/devices/default/pk_self.pem"),
    )
    .unwrap();
