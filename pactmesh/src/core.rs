...
#[derive(Parser, Debug)]
#[command(name = "pactmesh-core", author, version = PACTMESH_VERSION , about, long_about = None)]
struct Cli {
    #[arg(
        long,
        env = "ET_MACHINE_ID",
    ...
    #[clap(
        long,
        env = "PNW_EMPTY_START",
        help = "start with no network instance (live RPC portal, zero instances) so networks can be attached at runtime"
    )]
    empty: bool,
...
struct NetworkOptions {
    ...
    #[arg(
        long,
        env = "ET_BIND_DEVICE_PUBLIC",
        help = t!("core_clap.bind_device_public").to_string()
    )]
    bind_device_public: Option<bool>,
...
async fn run_main(cli: Cli) -> anyhow::Result<()> {
    ...
    for (config_file, source) in config_files {
        ...
        log::info!(
            "\
            Starting pactmesh from config file {:?}({:?}) with config:\n\
            ############### TOML ###############\n\
            {}\n\
            -----------------------------------\n\
            ",
    ...
    if crate_cli_network {
        ...
        log::info!(
            "\
            Starting pactmesh from cli with config:\n\
            ############### TOML ###############\n\
            {}\n\
            -----------------------------------\n\
            ",
...
pub async fn main() -> ExitCode {
    ...
    if let Some(shell) = cli.gen_autocomplete {
        ...
        if let Some(shell) = shell.to_shell() {
            crate::print_completions(shell, &mut cmd, "pactmesh-core");
        } else {
            // Handle Nushell
            crate::print_nushell_completions(&mut cmd, "pactmesh-core");
        }
