...
**CLI** (`pactmesh`):
```
pactmesh credential generate
  输出: credential_id=<公钥base64>  credential_secret=<私钥base64>

pactmesh credential revoke <credential_id>
pactmesh credential list
```

**临时节点启动**:
```bash
# 方式1: 直接传入凭据私钥
pactmesh-core --network-name test \
  --secure-mode \
  --credential <私钥base64> \
...
3. **手动测试**:
   ```bash
   # A: 管理节点
   pactmesh-core -n test -s secret --secure-mode --listeners tcp://0.0.0.0:11010
   pactmesh credential generate  # → credential_id + credential_secret

   # C: 临时节点
   pactmesh-core -n test --secure-mode --credential <私钥base64> --peers tcp://A:11010
...
pactmesh credential generate --groups guest,restricted --ttl 86400

# 生成允许 relay 的凭据，有效期 7 天
pactmesh credential generate --groups relay-node --allow-relay --ttl 604800

# 最简用法（默认 group 名为 "credential"）
pactmesh credential generate --ttl 3600
```
