...
use easytier::{
    dns::{hosts_writer::{HostnameIndexEntry, HostsWriter, NetworkKey}, platform::MockHostsBackend},
    trust::{DeviceFingerprint, MemberCert, NetworkLocalId, TrustDomainId},
};
use test_harness::*;

fn td_from_str(value: &str) -> TrustDomainId {
    let bytes = base64::Engine::decode(&base64::engine::general_purpose::URL_SAFE_NO_PAD, value).unwrap();
    TrustDomainId(bytes.try_into().unwrap())
}

fn network_key(trust_domain_id: &str, network: &str) -> NetworkKey {
    NetworkKey::new(td_from_str(trust_domain_id), NetworkLocalId::try_from_str(network).unwrap())
}
...
fn index_entry(cert: &MemberCert, hostname: &str, network: &str, trust_domain_id: &str) -> HostnameIndexEntry {
    HostnameIndexEntry {
...
fn ipam(fp_a: DeviceFingerprint, fp_b: DeviceFingerprint) -> impl Fn(&DeviceFingerprint) -> Option<IpAddr> {
    move |fingerprint| {
        if *fingerprint == fp_a {
...
async fn test_magicdns_hosts_contains_peer_fqdns_and_preserves_external_lines() {
    ...
    HostsWriter::refresh_with_backend(&backend, &indexes, &ipam(fingerprint(&cert_a), fingerprint(&cert_b)), true).unwrap();

    let content = backend.content();
    assert!(content.contains("1.2.3.4 external.example"));
    assert!(content.contains("10.144.0.10 laptop\n"));
    assert!(content.contains(&format!("10.144.0.10 laptop.{NETWORK_LOCAL_ID}.{}.pnw", &trust_domain_id[..8])));
    assert!(content.contains("10.144.0.20 server\n"));
    assert!(content.contains(&format!("10.144.0.20 server.{NETWORK_LOCAL_ID}.{}.pnw", &trust_domain_id[..8])));
}
...
async fn test_magicdns_rename_and_revoke_refreshes_hosts_block() {
    ...
    let mut indexes = BTreeMap::new();
    indexes.insert(key.clone(), vec![index_entry(&cert_a, "laptop", NETWORK_LOCAL_ID, &trust_domain_id), index_entry(&cert_b, "server", NETWORK_LOCAL_ID, &trust_domain_id)]);
    ...
    indexes.insert(key.clone(), vec![index_entry(&cert_a, "macbook", NETWORK_LOCAL_ID, &trust_domain_id), index_entry(&cert_b, "server", NETWORK_LOCAL_ID, &trust_domain_id)]);
    HostsWriter::refresh_with_backend(&backend, &indexes, &lookup, true).unwrap();
    assert!(backend.content().contains("macbook"));
    assert!(!backend.content().contains("laptop"));

    revoke_member(root_dir.path(), &trust_domain_id, &cert_b);
    indexes.insert(key, vec![index_entry(&cert_a, "macbook", NETWORK_LOCAL_ID, &trust_domain_id)]);
    HostsWriter::refresh_with_backend(&backend, &indexes, &lookup, true).unwrap();
    assert!(backend.content().contains("macbook"));
    assert!(!backend.content().contains("server"));
}
...
async fn test_magicdns_multi_network_disables_short_names() {
    ...
    indexes.insert(
        network_key(&trust_domain_id, NETWORK_LOCAL_ID),
        vec![index_entry(&cert_a, "laptop", NETWORK_LOCAL_ID, &trust_domain_id)],
    );
    indexes.insert(
    ...
    HostsWriter::refresh_with_backend(&backend, &indexes, &ipam(fingerprint(&cert_a), fingerprint(&cert_b)), false).unwrap();

    let content = backend.content();
    assert!(!content.contains("10.144.0.10 laptop\n"));
    assert!(!content.contains("10.144.0.20 server\n"));
    assert!(content.contains(&format!("laptop.{NETWORK_LOCAL_ID}.{}.pnw", &trust_domain_id[..8])));
