...
async fn test_receive_network_state_accepts_and_persists_newer_state() {
    ...
    let persisted =
        easytier::trust::SignedNetworkState::from_pem(&std::fs::read_to_string(path).unwrap())
            .unwrap();
...
async fn test_receive_network_state_rejects_stale_without_overwriting_disk() {
    ...
    assert!(matches!(
        err,
        NetworkStateReceiveError::PoolApply(easytier::trust::pool::PoolApplyError::StaleVersion {
            have: 2,
            got: 1
        })
    ));
    let persisted = easytier::trust::SignedNetworkState::from_pem(
...
async fn test_receive_network_state_rejects_wrong_scope_and_tamper() {
    ...
    assert!(matches!(
        err,
        NetworkStateReceiveError::PoolApply(easytier::trust::pool::PoolApplyError::BadSignature)
    ));
}
